...
|---|---|
| hax | `cryspen/hax` @ `5cdb74443a1eeebd34f7bae057ead44bc8a25107` (branch `proverif-rust-backend`, `cargo-hax-v0.3.7-280-g5cdb74443`) |
...
| `extraction-model/primitives.pvl` | A verbatim copy of hax's shipped prelude (no deviations — see below). | Yes — a copy, re-syncable from hax. |
...
### `primitives.pvl`: a copy of hax's shipped prelude

`extraction-model/primitives.pvl` is a verbatim copy of hax's shipped
`hax-lib/proof-libs/proverif/primitives.pvl`; it carries no SPQR-specific
deviation. The epoch model needs integer-typed comparison and `+ 1` to be
*decidable* (otherwise generated `assert_eq!(epoch, …)` guards never discharge
and every recv transition dead-ends). The shipped prelude supplies this via a
`nat2native` / `native2nat` bridge between the opaque `nat_N` literal constants
and ProVerif's built-in `nat`: `model.pvl` reads an extracted epoch with
`nat2native(...)` and then uses native `nat` tables, native `<` ordering, and the
canonical `+ 1` reduction (`epoch_succ`). Structural integer equality is already
in the shipped prelude. So the three changes SPQR used to vendor are now covered
by the shipped prelude, and this file is just a copy to keep the check
self-contained — re-sync it from hax when the prelude changes.
