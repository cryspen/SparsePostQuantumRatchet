...
PROVERIF_EXTRACTION_DIR = "proofs/proverif/extraction"
...
class verifyProverifAction(argparse.Action):
    ...
    def __call__(self, parser, args, values, option_string=None) -> None:
        ...
        if epochs is not None:
            ...
            nepochs = os.path.join(PROVERIF_EXTRACTION_DIR, "nepochs.pvl")
        ...
        # Libraries: `primitives.pvl` (vendored hax prelude with the
        # machine-int/nat-arithmetic fixes), `handwritten_lib.pvl` (symbolic
        # crypto), `lib.pvl` (generated protocol), `nepochs.pvl` (NEPOCHS bound,
        # before model.pvl which uses max_epoch()), and `model.pvl` (process
        # model + compromise). Each property is a separate query file.
        libs = [
            "-lib", "primitives.pvl",
